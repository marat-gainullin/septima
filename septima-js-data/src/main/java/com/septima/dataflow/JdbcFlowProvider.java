 ...
 * @see FlowProvider
 */
public abstract class JdbcFlowProvider<JKT> implements FlowProvider {
    ...
    */
    ...
    public <T> T select(Parameters aParams, Function<ResultSet, T> aResultSetProcessor, Consumer<T> onSuccess, Consumer<Exception> onFailure) throws Exception {
        if (lowLevelResults != null) {
            assert isPaged();
                ...
                try {
                    ...
                    if (stmt != null) {
                        try {
                            ...
                            if (rs != null) {
                                try {
                                    return aResultSetProcessor.apply(rs);
                                } finally {
                                    if (isPaged()) {
                                        lowLevelResults = rs;
                                    } else {
                                ...
                                return aResultSetProcessor.apply(null);
                            }
                        } catch (SQLException ex) {
                            throw new FlowProviderFailedException(ex);
                        } finally {
            ...
            if (finerLogs) {
                for (int i = 1; i <= aParams.getParametersCount(); i++) {
                    ...
                    if (paramValue != null && Scripts.DATE_TYPE_NAME.equals(param.getType())) {
                        ...
                        SimpleDateFormat sdf = new SimpleDateFormat(JsonUtils.DATE_FORMAT);
