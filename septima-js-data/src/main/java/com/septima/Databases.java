...
import com.septima.client.changes.EntitiesHost;
import com.septima.client.dataflow.ColumnsIndicies;
...
import com.septima.client.queries.SqlCompiledQuery;
...
public class Databases {
    ...
    // metadata
    protected Map<String, Metadata> mdCaches = new ConcurrentHashMap<>();
     ...
     * @throws Exception
     */
    public Databases(String aDefaultDatasourceName, boolean aAutoFillMetadata, int aMaxJdbcThreads) throws Exception {
        super();
        ...
        if (aDataSourceName != null && !aDataSourceName.isEmpty()) {
            ...
            try {
                ...
                try {
                    ...
                    // Platypus standalone server or databases
     ...
     * in flow providers, created by the databases.
     */
    ...
    /**
     * Factory method for JdbcFlowProvider. Intended to incapsulate flow
     ...
     * @throws Exception
     */
    public SeptimaFlowProvider createFlowProvider(String aDatasourceName, String aEntityName, String aSqlClause, Fields aExpectedFields) throws Exception {
        return new SeptimaFlowProvider(this, aDatasourceName, aEntityName, obtainDataSource(aDatasourceName), (Runnable aTask) -> {
            startJdbcTask(aTask);
    ...
    public static Map<String, String> getUserProperties(Databases aClient, String aUserName, Scripts.Space aSpace, Consumer<Map<String, String>> onSuccess, Consumer<Exception> onFailure) throws Exception {
    ...
    public static PlatypusPrincipal credentialsToPrincipalWithBasicAuthentication(Databases aClient, String aUserName, String aPassword, Scripts.Space aSpace, Consumer<PlatypusPrincipal> aOnSuccess, Consumer<Exception> aOnFailure) throws Exception {
    ...
    public static PlatypusPrincipal userNameToPrincipal(Databases aClient, String aUserName, Scripts.Space aSpace, Consumer<PlatypusPrincipal> aOnSuccess, Consumer<Exception> aOnFailure) throws Exception {
    ...
    public Metadata getMetadataCache(String aDatasourceName) throws Exception {
        if (aDatasourceName == null) {
            aDatasourceName = defaultDatasourceName;
        }
        if (aDatasourceName != null) {
            if (!mdCaches.containsKey(aDatasourceName)) {
                Metadata cache = new Metadata(this, aDatasourceName);
                if (autoFillMetadata) {
                    try {
                        cache.fillTablesCacheByConnectionSchema();
                    } catch (NamingException ex) {
                        Logger.getLogger(Databases.class.getName()).log(Level.WARNING, ex.getMessage());
                    }
                }
    ...
    protected ApplyResult apply(final String aDatasourceName, List<Change.Applicable> aLog) throws Exception {
        int rowsAffected;
        Metadata mdCache = getMetadataCache(aDatasourceName);
        if (mdCache == null) {
                    ...
                    public Field resolveField(String aEntityName, String aFieldName) throws Exception {
                        if (aEntityName != null) {
                            ...
                            if (fields != null) {
                                ...
                                Logger.getLogger(Databases.class.getName()).log(Level.WARNING, "Cant find fields for entity id:{0}", aEntityName);
                                return null;
                            }
                        } else {
                            return null;
    ...
    public void tableChanged(String aDatasourceName, String aTable) throws Exception {
        Metadata cache = getMetadataCache(aDatasourceName);
    ...
    protected static Set<String> getUserRoles(Databases aClient, String aUserName, Scripts.Space aSpace, Consumer<Set<String>> onSuccess, Consumer<Exception> onFailure) throws Exception {
